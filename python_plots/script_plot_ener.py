...
# 3 - read data                              
# -------------------------------------------
fener = open('energetics.out','r')
ener_names = fener.readline().split()
ener_lines = fener.readlines()
...
for ener_name in ener_names:
   ener_data[ener_name] = np.ma.zeros(len(ener_lines), 'f', fill_value = -999.999)

for (line_count, line) in enumerate(ener_lines):
    items = line.split()
    for (col_count, col_name) in enumerate(ener_names):
        value = items[col_count]
        ener_data[col_name][line_count] = checkValue(value)
...
plt.plot(phys_data[xaxis], phys_data["Vn"]/1e5,     '-', c='blue',    alpha=1.0, markeredgecolor='none',markersize=msize,label='V$_n$')
plt.plot(phys_data[xaxis], phys_data["Vi"]/1e5,     '-', c='orange',  alpha=1.0, markeredgecolor='none',markersize=msize,label='V$_i$')
plt.plot(phys_data[xaxis], phys_data["Vsound"]/1e5, '-', c='black',   alpha=1.0, markeredgecolor='none',markersize=msize,label='V$_s$')
plt.plot(phys_data[xaxis], phys_data["Vmagnet"]/1e5,'-', c='green',   alpha=1.0, markeredgecolor='none',markersize=msize,label='V$_m$')
plt.plot(phys_data[xaxis], phys_data["VAlfven"]/1e5,'-', c='magenta', alpha=1.0, markeredgecolor='none',markersize=msize,label='V$_a$')
plt.legend(loc='lower right', borderaxespad=0., fontsize=tsize)

plt.xlabel(xlabel, fontsize=tsize)
plt.ylabel('velocity (km s$^{-1}$)', fontsize=tsize)
...
plt.plot(chem_data[xaxis], ener_data["Energy"] - ener_data["Energy_int"] - ener_data["Energ_gain"],       '-', c='black',  alpha=1.0, markeredgecolor='none',markersize=msize,label='tot')
plt.plot(chem_data[xaxis], ener_data["Energy_kin"], '-', c='red',     alpha=1.0, markeredgecolor='none',markersize=msize,label='kine')
plt.plot(chem_data[xaxis], ener_data["Energy_the"], '-', c='blue',    alpha=1.0, markeredgecolor='none',markersize=msize,label='ther')
plt.plot(chem_data[xaxis], ener_data["Energy_mag"], '-', c='green',   alpha=1.0, markeredgecolor='none',markersize=msize,label='magn')
plt.plot(chem_data[xaxis], ener_data["Energy_vis"], '-', c='magenta', alpha=1.0, markeredgecolor='none',markersize=msize,label='visc')
plt.plot(chem_data[xaxis],-ener_data["Energ_gain"], '-', c='orange',  alpha=1.0, markeredgecolor='none',markersize=msize,label='loss')
###plt.plot(chem_data[xaxis], ener_data["x(HCO+)"],    '-', c='gold',   alpha=1.0, markeredgecolor='none',markersize=msize,label='x(HCO$^+$)')
###plt.plot(chem_data[xaxis], ener_data["x(ELECTR)"],  '-', c='grey',   alpha=1.0, markeredgecolor='none',markersize=msize,label='x(e$^-$)')
#plt.legend(loc='best', borderaxespad=0., fontsize=tsize)
plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left', borderaxespad=0., fontsize=tsize)

plt.xlabel(xlabel, fontsize=tsize)
plt.ylabel('energy flux (erg cm$^{-2}$ s$^{-1}$)', fontsize=tsize)
